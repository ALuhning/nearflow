...
class RegisterNearAIAgentTool(LCToolComponent):
    ...
    outputs = [Output(name="agent_id_out", display_name="Agent ID", method="build")]
    ...
    async def build(self) -> str:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            find_resp = await client.post(f"{base_url}/find_agents", headers=headers, json=find_payload)
        ...
        matching_agent = next(
            (
                ...
                if agent.get("name") == self.agent_name and agent.get("namespace") == owner_id
            ),
            None,
        )
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(f"{base_url}/registry/upload_metadata", headers=headers, json=payload)
