    ...
    def build_store(self) -> Data:
        ...
        if provided_id and clear_existing:
            ...
            if list_response.status_code != 200:
                raise ValueError(
                    f"[CLEAR ERROR] Failed to list files: {list_response.status_code} - {list_response.text}"
                )
            ...
            for file_info in file_list:
                ...
                if detach_response.status_code not in (200, 204):
                    print(
                        f"[WARN] Failed to detach file {file_id}: {detach_response.status_code} - {detach_response.text}"
                    )
                else:
        ...
        # --- CASE 3: Provided ID but not clearing, validate and fallback to create ---
            ...
            if check_response.status_code == 200:
                ...
                raise ValueError(
                    f"Error validating vector store {provided_id}: {check_response.status_code} - {check_response.text}"
                )

        # --- CASE 4: No ID provided, create new store ---
        elif not provided_id:
            print("[CREATE] No vector store ID provided, creating a new one.")
            create_url = f"{self.nearai_api_base}/vector_stores"
            create_response = requests.post(create_url, headers=headers_json, json={"name": store_name})
            create_response.raise_for_status()
            self.vector_store_id = create_response.json()["id"]
        ...
        for i, data_obj in enumerate(self.ingest_data):
            ...
            ext = {"pdf": ".pdf", "md": ".md", "markdown": ".md"}.get(file_type, ".txt")

            mime_type = {".pdf": "application/pdf", ".md": "text/markdown", ".txt": "text/plain"}.get(ext, "text/plain")

            print(f"[UPLOAD] Uploading file_{i}{ext} to /v1/files as {mime_type}")

            files = {"file": (f"file_{i}{ext}", text.encode("utf-8"), mime_type), "purpose": (None, "assistants")}

            upload_response = requests.post(
                f"{self.nearai_api_base}/files", headers={"Authorization": f"Bearer {api_key}"}, files=files
            )
            ...
            attach_response = requests.post(attach_url, headers=headers_json, json={"file_id": file_id})
