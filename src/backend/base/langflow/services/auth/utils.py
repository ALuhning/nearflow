...
async def authenticate_user_with_near_staking(username: str, password: str, db: AsyncSession) -> User | None:
    ...
    try:
        ...
        logger.info(f"NEAR staking verification successful for {username}: {staking_result['stake_amount']} NEAR staked")
        return user
...
async def authenticate_near_account(near_account_id: str, db: AsyncSession) -> tuple[User, bool, str]:
    ...
    if not settings_service.auth_settings.ENABLE_NEAR_STAKING_VERIFICATION:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="NEAR account authentication is not enabled"
        )
    ...
    try:
        ...
        if not is_designated_superuser:
            ...
            if not staking_result["is_staker"]:
                logger.warning(f"NEAR account {near_account_id} is not a staker in {settings_service.auth_settings.NEAR_POOL_CONTRACT}")
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"Access denied: You must be a staker in {settings_service.auth_settings.NEAR_POOL_CONTRACT} to access this service"
                )
            ...
            if not staking_result["meets_minimum"]:
                min_stake = settings_service.auth_settings.NEAR_MIN_STAKE_AMOUNT
                actual_stake = staking_result["stake_amount"]
                logger.warning(f"NEAR account {near_account_id} stake ({actual_stake} NEAR) is below minimum ({min_stake} NEAR)")
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"Access denied: Minimum stake of {min_stake} NEAR required. Your current stake: {actual_stake} NEAR"
                )
...
async def verify_near_signature(
    ...
    try:
        ...
        key_belongs_to_user = await verify_full_key_belongs_to_user(
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if "result" in data and "keys" in data["result"]:
                            ...
                            logger.error(f"Ownership verification failed: Invalid key or accountId. Account {account_id} provided key {public_key} but available keys are: {available_keys}")
                            raise ValueError(f"Ownership verification failed: Invalid key or accountId. The public key provided by your wallet does not match any of the keys associated with account {account_id}. This commonly occurs with hardware wallet connections.")
            except Exception as inner_e:
                logger.error(f"Error getting key details for error reporting: {inner_e}")
            
            raise ValueError(f"Ownership verification failed: Invalid key or accountId for account {account_id}. Key: {public_key}")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for key_info in data["result"]["keys"]:
                ...
                logger.debug(f"Checking stored key: {stored_key} against provided key: {public_key} (normalized: {normalized_provided_key})")
                ...
                stored_key_base = stored_key.replace('ed25519:', '') if stored_key.startswith('ed25519:') else stored_key
                provided_key_base = public_key.replace('ed25519:', '') if public_key.startswith('ed25519:') else public_key
                if stored_key_base == provided_key_base:
                    is_full_access = key_info["access_key"]["permission"] == "FullAccess"
                    logger.debug(f"Public key {public_key} matched (without prefix) for {account_id}, full access: {is_full_access}")
                    return is_full_access
        ...
        async with httpx.AsyncClient() as client:
            ...
            for key_info in data["result"]["keys"]:
                if key_info["public_key"] == public_key:
                    ...
                    logger.debug(f"Permission: {permission}, FullAccess: {is_full_access}, FunctionCall: {is_function_call}")
...
async def authenticate_near_account_with_signature(
    ...
    try:
        ...
        if settings_service.auth_settings.NEAR_DEV_MODE:
            ...
            try:
                ...
                if decoded_sig.startswith('ledger_proof:'):
                    ...
                    if public_key == "LEDGER_AUTO_DETECT":
        ...
        if settings_service.auth_settings.ENABLE_NEAR_STAKING_VERIFICATION and not settings_service.auth_settings.NEAR_DEV_MODE and not is_designated_superuser:
            # Configure the staking verifier with current settings
            near_staking_verifier.update_settings(
                rpc_url=settings_service.auth_settings.NEAR_RPC_URL,
                pool_contract=settings_service.auth_settings.NEAR_POOL_CONTRACT,
            ...
            if not is_staker:
                ...
                raise ValueError(f"Account {account_id} does not meet minimum staking requirements. Please stake at least {settings_service.auth_settings.NEAR_MIN_STAKE_AMOUNT} NEAR with vitalpoint.pool.near")
        ...
        async with httpx.AsyncClient() as client:
            ...
            for key_info in data["result"]["keys"]:
                ...
                if is_full_access or is_function_call:
                    ...
                    logger.debug(f"Available key for auto-detection: {key_info['public_key']} (permission: {permission})")
