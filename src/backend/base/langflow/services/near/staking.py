...
class NEARStakingVerifier:
    ...
    async def verify_staker(self, account_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if stake_info is None:
                return {"is_staker": False, "stake_amount": Decimal("0"), "meets_minimum": False, "error": None}
            ...
            logger.info(
                f"NEAR staking verification for {account_id}: stake={stake_amount} NEAR, minimum={self.min_stake_amount} NEAR"
            )

            return {"is_staker": True, "stake_amount": stake_amount, "meets_minimum": meets_minimum, "error": None}

        except Exception as e:
            logger.error(f"Error verifying NEAR staker {account_id}: {e}")
            return {"is_staker": False, "stake_amount": Decimal("0"), "meets_minimum": False, "error": str(e)}

    async def _get_staker_info(self, account_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            response = await self._call_contract_method(
                contract_id=self.pool_contract, method_name="get_account", args={"account_id": account_id}
            )
    ...
    async def _call_contract_method(
        self, contract_id: str, method_name: str, args: dict[str, Any]
    ) -> dict[str, Any] | None:
